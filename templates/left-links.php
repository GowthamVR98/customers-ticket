...
function getDynamicLinks($dbh,$parentId = NULL){
      ...
      $status = 1;
      if(is_null($parentId)){
        $getDynamicLink = "SELECT * from ".TBL_PRIFIX."dynamic_left_links WHERE status =:status AND parent_id IS :dynamic_id ORDER BY priority ASC";
        $query = $dbh->prepare($getDynamicLink);
        $query->bindParam(':status',$status);
        $query->bindParam(':dynamic_id',$parentId);
      }else{
        $getDynamicLink = "SELECT * from ".TBL_PRIFIX."dynamic_left_links WHERE status =:status AND parent_id = :dynamic_id ORDER BY priority ASC";
        $query = $dbh->prepare($getDynamicLink);
        $query->bindParam(':status',$status);
        $query->bindParam(':dynamic_id',$parentId);
      }
...
<section>
        ...
        <aside id="leftsidebar" class="sidebar">
            ...
            <?php
                ...
                $getLoggedData = $dbobj->selectData($dbh, TBL_PRIFIX.'user_login_details', array('profile_pic','fname','concat_ws(" ",fname,lname) as fullName','email_id','mobile_number'), 'user_id = ?', array($loginSessionId));
